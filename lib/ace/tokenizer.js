...
var Tokenizer = function(rules, flag) {
    ...
    for ( var key in this.rules) {
        ...
        for ( var i = 0; i < state.length; i++) {
            ...
            if (RegExp(adjustedregex).test("")) {
                console.log(state[i].regex, state[i].next, key)
            }
            
            if (RegExp(adjustedregex).test("") && mapping.empty == null && state[i].next)
                mapping.empty = i
        ...
        while (match = re.exec(line)) {
            ...
            if (line && !value[0]) {
                ...
                for (var i = 0; i < match.length-2; i++) {
                    if (match[i + 1] === undefined || (match[i + 1] == "" && value[0]))
                        continue;

                    rule = state[mapping[i].ruleIndex];
